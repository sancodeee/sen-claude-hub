...
- **台账是磁盘文件，不是对话记忆。** 位置：`travel-guides/<目的地>-<出行首日>-research.md`，`<目的地>` 写法必须与最终 HTML 文件名中的目的地完全一致（同为中文或同为拼音），保证两文件肉眼可配对；schema 复用证据卡片字段（可表格化，与 `template.md` 来源附录表结构对齐，便于直接生成附录）。默认保留该文件供交付后回溯，用户可自行删除。
- **照抄铁律**：分段生成阶段所有易变数值从台账文件照抄，禁凭记忆改写；台账中标 `⚠️` / `❓` 的信息不得在下游被升级为确定值。
- **数值不落台账不得进正文**：正文拟引用的每一个具体数值（车次时刻、价格、时长、开放时间、预约规则）都必须先存在于台账；把证据卡片汇入台账时**不得丢弃卡片中的数值型字段**（如只记票价丢掉时刻）——丢了字段，照抄铁律就没有可抄的源头。
- **跨域冲突必须合并裁决**：多个 Research Subagent 的调研范围会天然重叠（如「采耳价格」同时出现在餐饮域与避坑域）。汇入台账时必须按「信息项」跨域去重；同一信息项多域数值不一致时，必须当场裁决为一个口径（写明裁决依据）或标 `⚠️` 注明区间，**不得两个口径并存入账**——并存的口径会被分别抄进正文不同段落，制造自相矛盾。
...
4. Main Orchestrator 将证据合并落盘为事实台账文件，执行双源核验、UGC 降权、**跨域按信息项去重裁决**、冲突处理和置信度裁决；不丢弃卡片中的数值型字段。
...
7. Final Verification Agent（输入：HTML 文件路径 + 台账文件路径，含高风险信息联网抽查）输出最终核验报告；若不通过，Main Orchestrator 按下节「修复与重新核验」处理。

## 修复与重新核验

- **事实错误必须同步修台账**：质检（含联网抽查）纠出的事实错误，源头往往在台账——只改正文不改台账，过时结论会留在台账里误导交付后的复查。修复时必须**正文与台账同步修订**，并在台账追加一行修订记录（改了什么、依据什么来源、修订日期）。
- **复核分级，不一律重派**：
  - 一致性/格式类修复（措辞矛盾、数值统一、卡片补要素）：由 Main Orchestrator 对**每个修复点定点复核**（如 grep 确认旧口径残留为 0、新口径已落全部位置）即可放行；
  - **阻断级事实错误**修复后：必须重新派发 Final Verification Agent（或至少对该信息项重新联网复核）确认新口径成立，方可放行。
- 复核结果须在交付说明中如实报告：修了什么、如何复核、是否放行。
