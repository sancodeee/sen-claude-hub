...
- `assets/`：后端与全栈正式文档骨架。
- `validate_design_document.py`：检查可机械验证的格式、非空语义块和结构化引用规则，不替代业务语义审查。
...
正式文档采用“核心正文 + 条件内容块 + 契约附录”，正文一级标题与 YAML `title` 一致，随后通过 summary callout 给出定位与核心结论。公共语义收敛到以下阅读顺序：

1. 设计概览：目标、范围、非目标、设计依据、当前与目标差异、必要术语及关键决策。
2. 总体架构与职责：模块或服务边界、数据归属和真实依赖。
3. 全流程与交互设计：闭环导航、`FLOW-xx`、适用的页面或业务状态。
4. 安全、一致性与异常恢复：权限、敏感信息、幂等、并发、事务、`ERR-xx`、重试和补偿。
5. 验收、风险与就绪结论：`AC-xx`、反向场景、残余风险和最终结论。
6. 契约附录：实际涉及的 `API-xx`、`DATA-xx` 及跨层字段映射；不涉及接口或数据模型依赖时分别给出一句可核验结论。

全栈模式在架构与流程之间增加前端设计。中间件、外部系统、状态机、分布式一致性和特定图形均为条件内容；不适用时集中说明依据，不保留空章节、空表或虚假编号。接口或数据模型依赖即使不涉及，也在契约附录各用一句结论闭合审查语义。上述语义必须完整，但不要求每个维度独占二级章节。
...
每条后端业务链路使用唯一 `FLOW-xx` 并关联 `AC-xx`；实际涉及的接口、数据和统一错误策略再通过 `API/DATA/ERR` 编号关联，不在横切章节重复链路正文。
...
每项用户操作归入一个 `FLOW-xx` 并关联 `AC-xx`；页面状态和业务状态分别定义，实际涉及的接口、字段和错误提示通过编号引用同一事实源。

## 8. 接口与数据精度

### 8.1 接口契约

每个同步接口、RPC、事件、任务或回调 `API-xx` 单独明确：

- 契约名称、精确 URL/RPC/Topic/事件或任务标识、HTTP Method 或投递方式、调用方和使用场景。
- 消息或任务的 schema/version、key、消息头、确认时机和投递语义。
- 鉴权、权限和调用前置条件。
- 适用的 Header、Path、Query、Body、消息头或消息体字段。
- 字段名称、类型、必填性、长度、格式、枚举、约束、业务语义和示例。
- 返回结构、全部字段、空值语义和分页规则。
- 错误码、触发条件和调用方处理。
- 幂等规则，关联的 `FLOW`，以及实际涉及的 `DATA/ERR` 编号；状态变化、数据影响和后续动作由 `FLOW-xx` 定义一次。
- 仅在字段组合关系难以由字段表理解时附精简请求或响应示例；示例不能代替字段表。

### 8.2 数据库契约

每个新增、调整或被关键链路依赖的 `DATA-xx` 明确：
...
新增或调整字段、约束、索引、状态时，文档不提供可执行 DDL，但字段定义必须足以让后续实现设计准确生成 DDL。纯复用既有模型且无上述变化时，不复制全量表结构；引用已核验来源，并由跨层映射列出本设计实际使用的字段和影响设计的约束。

## 9. 流程图要求

Mermaid 只在表格与短正文无法清楚表达参与方顺序、条件分支、合法状态迁移或实体关系时使用。模板不固定预置所有图形。

根据实际场景使用：

- 多模块、服务或数据归属复杂时使用总体架构图。
- 多参与方存在重要顺序时使用主业务链路时序图。
- 关键异常或恢复分支复杂时使用流程图。
- 存在受约束的页面或业务状态迁移时使用状态机图。
- 多实体关系或基数影响实现时使用实体关系图。

同一简单链路不同时生成时序图和流程图。图中的参与方、节点、接口、状态和顺序必须与正文、接口契约和数据模型一致。不得为了装饰生成与设计无关的图。

## 10. 闭环与一致性审查

### 10.1 需求覆盖

只对影响设计的核心需求建立精简映射，通过闭环导航关联 `FLOW`、`AC`，以及实际涉及的契约、数据或状态和 `ERR`。
...
闭环导航表可以合并列，但每行必须覆盖上述语义，以 `FLOW-xx` 开头并引用 `AC-xx`；涉及接口、数据或统一错误策略时引用 `API-xx`、`DATA-xx`、`ERR-xx`。编号定义必须是以编号开头的 H2–H6 标题；表格和行内出现只算引用。每个编号只能定义一次且必须有实质内容。

### 10.3 接口闭环

- 每个前端动作或服务调用有对应接口或明确的本地动作。
- 每个接口有明确调用方和结果消费方。
...
- 需要跨流程或接口复用的后端错误通过唯一 `ERR-xx` 转换为明确用户反馈；简单局部失败在所属 `FLOW-xx` 中直接定义恢复规则。
- 前端不依赖接口未提供的信息。
...
YAML 结束后先写与 `title` 完全一致的正文一级标题，再使用 `[!summary]` 说明文档定位，并按实际需要使用 `[!important]`、`[!note]`、`[!warning]` 和 `[!success]`。普通块引用不能替代 YAML 元数据；“本文件仅为骨架”“落盘门禁”等模板作者说明不得进入成熟文档。

更新已有设计时：

- 先建立“原位保留、合并到新正文、改为编号引用、依据变化而移除”的迁移映射，不直接用新模板覆盖原文。
- 保留仍然有效的历史结论和双链。
- 保持原文件路径、`title` 与 `created`，归一化后的双链目标不得无故减少。
- 更新 `version`、`updated`、`last_verified` 和成熟度。
- 在正文开头增加精简校正记录。
- 已解决事项移入“已确认”或“已校正”章节。
- 若重新核验产生真正未解决的问题，停止成熟文档交付并返回阻塞问题，不在正式文档中保留待确认章节。
...
- YAML Frontmatter 存在，必填字段唯一，必填标量值非空。
- 成熟度字段为正式交付值。
- 不存在未替换占位符。
- 成熟文档不包含“模板骨架”“落盘门禁”等模板编写标记。
- 正文一级标题存在并与 YAML `title` 一致。
- 需要的公共语义章节存在且具有实质内容。
- 每张保留的表格表头、分隔行和数据行列数一致，且至少包含一行有效数据。
- 全文只包含一张闭环导航表；每行单元格非空、首列以 `FLOW` 开头，并在验收列关联 `AC`。
- 文中实际出现的 `FLOW/API/DATA/ERR/AC` 引用均有唯一且非空的定义。
- Mermaid 代码块完整闭合。
- 全栈模式具有内容非空的 H2 前端设计章节。
- 正文双链全部登记到 `related`，且 `related` 不包含正文未使用项。
- 正式文档不存在影响实现的待确认标记。

“实质内容”是已核验正文或至少一行真实表格数据；空标题、空表、裸 callout、HTML 注释或只有 Mermaid 图不算。脚本只验证闭环载体和引用完整性，不判断编号内容的业务正确性、条件图是否必要或设计是否合理。语义门禁仍由技能依据本规格第 10 节执行。
...
- 简单后端和只读全栈场景是否只保留适用内容块，不机械展开空章节和全部图形。
- 闭环导航是否至少关联唯一、非空且可解析的 `FLOW/AC`，实际引用的 `API/DATA/ERR` 是否同样可解析。
- 接口字段不完整时是否拒绝生成正式文档。
- 更新已有文档时是否正确维护 YAML、版本、双链和校正记录。
- 受到“先快速写一版”压力时是否仍拒绝猜测和半成品交付。
- 用户要求具体代码时是否守住详细设计与实现方案边界。
- 占位符、断裂 Mermaid、缺失章节或双链不一致能否被机械校验拦截。
- 正文 H1 缺失、重复或与 YAML 标题不一致能否被拦截。
- 空语义块、模板作者说明、重复或未定义编号能否被拦截。
...
- 正文采用少量核心章节，简单场景不机械保留空内容块或全部图形。
- 正式文档具备实际涉及的字段级接口与数据库定义；不涉及项有可核验结论。
- 闭环导航每行可以追溯到唯一且非空的 `FLOW/AC` 定义，实际引用的 `API/DATA/ERR` 也具备唯一且非空的定义。
- 达到条件图选择阈值的复杂流程使用适当 Mermaid 图表达，并通过图文一致性审查；短表和正文已足够时不强制绘图。
